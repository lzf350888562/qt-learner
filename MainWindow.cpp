...
{
    isErasing=0;
    pix=QPixmap(1281,701);
    pix.fill(Qt::white);
    pixTmp=QPixmap(1281,701);
...
bool MainWindow::eventFilter(QObject *obj, QEvent *e)
{
   ...
   return QWidget::eventFilter(obj,e);     //将事件传递给父类
}
...
void MainWindow::mousePressEvent(QMouseEvent *e)
{
    ...
    {
        lastPt=e->pos();
        lastPt.setY(lastPt.y()-125);  //向下移动
        lastPt.setX(lastPt.x()-35);
        isDrawing=1;                //左键按下
    }
}
...
void MainWindow::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if(e->buttons()==Qt::LeftButton)   // 获取按钮组 而不是按钮
    {
        ...
        endPt.setY(endPt.y()-125);
        endPt.setX(endPt.x()-35);
        isDrawing=2;            //按下并拖动
        update();        //进行绘制
    }
}
...
void MainWindow::on_tab1_Video_clicked()
{
    videoPlayer=new VideoPlayer(currentChapter);
    videoPlayer->show();
}
